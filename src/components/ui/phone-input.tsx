...
const PhoneInput: React.ForwardRefExoticComponent<PhoneInputProps> =
  React.forwardRef<React.ElementRef<typeof RPNInput.default>, PhoneInputProps>(
    ({ className, onChange, value, ...props }, ref) => {
      // Convert raw phone number to E.164 format for the component
      ...
      return (
        <RPNInput.default
          ...
          onChange={(newValue) => {
            if (newValue) {
              // Remove country code and return only the raw digits
              const rawNumber = newValue.replace(/^\+91/, '').replace(/\D/g, '');
              onChange?.(rawNumber);
            } else {
              onChange?.('');
            }
