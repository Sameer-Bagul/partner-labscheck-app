...
const LocationSelector = ({
  ...
  // Filter available states for the selected country
  const availableStates = statesData.filter(
    (state) => Number(state.country_id) === Number(selectedCountry?.id)
  );

  // 🟢 Set default values for Country and State on load
  useEffect(() => {
    if (defaultCountry) {
  ...
  }, [defaultCountry, defaultState, selectedCountry, availableStates,countriesData]);

  const handleCountrySelect = (country: CountryProps | null) => {
    setSelectedCountry(country);
    setSelectedState(null); // Reset state when country changes
    onCountryChange?.(country);
    onStateChange?.(null); // Clear state when changing country
  };
